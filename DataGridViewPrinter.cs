...
class DataGridViewPrinter
{
    ...
    private void Calculate(Graphics g)
    {
        if (PageNumber == 0) // Just calculate once
        {
            ...
            for (int i = 0; i < TheDataGridView.Columns.Count; i++)
            {
                ...
                //ColumnsWidth.Add(tmpWidth); // эта строка была изначально, теперь которая ниже
                ColumnsWidth.Add(TheDataGridView.Columns[i].Width);
            }
            TheDataGridViewWidth = 1100;//изменение внес я
    ...
    private void DrawHeader(Graphics g)
    {
        ...
        if (IsWithTitle)
        {
            ...
            //TitleFormat.FormatFlags = StringFormatFlags.LineLimit | StringFormatFlags.NoClip;
            if (IsCenterOnPage)
                TitleFormat.Alignment = StringAlignment.Center;
            else
                TitleFormat.Alignment = StringAlignment.Near;

            RectangleF TitleRectangle = new RectangleF((float)LeftMargin, CurrentY, (float)PageWidth - (float)RightMargin - (float)LeftMargin, g.MeasureString(TheTitleText, TheTitleFont).Height + g.MeasureString(TheTitleText, TheTitleFont).Height);

            g.DrawString(TheTitleText, TheTitleFont, new SolidBrush(TheTitleColor), TitleRectangle, TitleFormat);

            CurrentY += g.MeasureString(TheTitleText, TheTitleFont).Height + g.MeasureString(TheTitleText, TheTitleFont).Height;
        }
        ...
        RectangleF HeaderBounds = new RectangleF(CurrentX, CurrentY, mColumnPointsWidth[mColumnPoint],40 /*RowHeaderHeight*/);
        ...
        for (int i = (int)mColumnPoints[mColumnPoint].GetValue(0); i < (int)mColumnPoints[mColumnPoint].GetValue(1); i++)
        {
            ...
            CellBounds = new RectangleF(CurrentX, CurrentY, ColumnWidth, 40/*RowHeaderHeight*/);
            ...
            if (TheDataGridView.RowHeadersBorderStyle != DataGridViewHeaderBorderStyle.None) // Draw the cell border only if the HeaderBorderStyle is not None
                g.DrawRectangle(TheLinePen, CurrentX, CurrentY, ColumnWidth, 40/*RowHeaderHeight*/);

            CurrentX += ColumnWidth;
        }

        CurrentY += 40;// RowHeaderHeight;
    }
    ...
    private bool DrawRows(Graphics g)
    {
        ...
        if (CurrentRow == TheDataGridView.Rows.Count)
            return false;
        CurrentRow = 0;
